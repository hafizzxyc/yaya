...
display_welcome() {
  ...
  echo -e "${BLUE}[+]                   Created By Apiis              [+]${NC}"
  echo -e "${BLUE}[+]                                                 [+]${NC}"
  echo -e "${RED}[+] =============================================== [+]${NC}"
  echo -e ""
...
check_token() {
  ...
  if [ "$USER_TOKEN" = "apiis2024" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Skyzopedia${NC}"
    echo -e "${YELLOW}TELEGRAM : @apiisbeggard${NC}"
    echo -e "${YELLOW}WHATSAPP : +6281360773542${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© Apiis.Js${NC}"
    exit 1
  fi
  clear
}
